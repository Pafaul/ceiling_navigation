...
def calculate_keypoints_on_image(keypoints: list, camera: Camera):
    ...
    for keypoint in keypoints:
        ...
        x = int(camera.resolution[0] / 2 - camera.f * (nominator_x / denominator_x) / camera.px_mm[0])
        y = int(camera.resolution[1] / 2 - camera.f * (nominator_y / denominator_y) / camera.px_mm[1])
