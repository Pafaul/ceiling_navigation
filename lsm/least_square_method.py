...
def get_error(A: np.ndarray, B: np.array, X: np.array, calculate_error):
    ...
    :param B:
    :param X:
    :param calculate_error: method used to calculate error
    :return: error
    """
...
def seq_lsm(pts1, pts2, rotation_matrix, dist_threshold=35):
    """
    Least squares method for sequential frames
    :param pts1: keypoints of the first frame
    :param pts2: keypoints of the second frame
    :param rotation_matrix: rotation matrix between two frames frame1->frame2
    ...
    error_DP = [sum([((delta - ev) ** 2) / (elements_amount - 1) for delta in error]) for ev, error in
                zip(error_EV, errors)]
...
def map_lsm(pts1, pts2, dist_threshold=35):
    """
    Least squares method for map navigation
    :param pts1:
    :param pts2:
